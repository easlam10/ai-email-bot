// test-morning-direct.js
// Direct test of morning run email fetching without database operations
...
const testMorningRunDirect = async () => {
  try {
    console.log("🧪 Testing 10am PKT run (Morning) - Direct API test\n");
    ...
    todayUTC.setUTCHours(5, 0, 0, 0); // 10am PKT = 5am UTC
    
    // Set to 4pm PKT = 11am UTC (yesterday)
    const yesterdayUTC = new Date(todayUTC);
    yesterdayUTC.setUTCDate(yesterdayUTC.getUTCDate() - 1);
    yesterdayUTC.setUTCHours(11, 0, 0, 0); // 4pm PKT = 11am UTC
    
    const startUTC = yesterdayUTC;
    const endUTC = todayUTC;
    
    const startISO = startUTC.toISOString();
    const endISO = endUTC.toISOString();

    console.log(`🕙 Test simulates running at: 10am PKT (5am UTC)`);
    ...
    console.log(`   Expected: 4pm PKT yesterday to 10am PKT today\n`);
    ...
    if (mailResponse.data.value.length > 0) {
      ...
      // Save to JSON file
      const outputFile = `md-morning-run-emails.json`;
      fs.writeFileSync(outputFile, JSON.stringify({
        testInfo: {
          testDescription: "Morning run at 10am PKT (5am UTC)",
          expectedTimeRange: "4pm PKT yesterday to 10am PKT today",
...
// Run the test
testMorningRunDirect()
  .then(() => {
    console.log("\n✅ Morning run direct test completed!");
    process.exit(0);
  })
  .catch((error) => {
    console.error("\n❌ Test failed:", error);
